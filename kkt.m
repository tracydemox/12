...
for k=1:itr
    ...
    x_k = value(x);
     ...
     %+++++++++++++++++++++++++ kkt条件
     SP_Obj = c'*y_k(:,k);
     SP_cons=[SP_cons, D*y_k(:,k) >= d,...
         K*y_k(:,k) == g,...
         F*x_k + G*y_k(:,k) >= h,...
         I_u*y_k(:,k) == u,...
         y_k(:,k) >= 0 ];       % 原始可行性
     
%      SP_cons=[SP_cons, gamma>=0, nu>=0, miu>=0];  % 对偶可行性
     SP_cons=[SP_cons, gamma>=0, nu>=0];  % 对偶可行性
%      SP_cons=[SP_cons,  c - D'*gamma - K'*lamda - G'*nu - I_u'*pai - miu == 0 ];  % 稳定性条件
     SP_cons=[SP_cons,  c - D'*gamma - K'*lamda - G'*nu - I_u'*pai  >= 0 ];
     
     %%%%%%%% SP_cons=[SP_cons, (d-D*y_k(:,k))'*gamma==0, (h-G*y_k(:,k)-F*x_k)'*nu==0, y_k(:,k)'*miu==0];  % 互补松弛条件
     
%      SP_cons=[SP_cons, miu<=M*q, -y_k(:,k)>=M*(q-1)];
     SP_cons=[SP_cons, y_k(:,k)<=M*q,  D'*gamma + K'*lamda + G'*nu + I_u'*pai - c>=M*(q-1)];
     SP_cons=[SP_cons, gamma<=M*v, d-D*y_k(:,k)>=M*(v-1)];
     SP_cons=[SP_cons, nu<=M*w, h-G*y_k(:,k)-F*x_k>=M*(w-1)];
     
     SP_cons=[SP_cons, u == u_hat + du.*B,  sum(B_pv) <= Gamma_pv, sum(B_L) <= Gamma_L];
...
disp("------------------------")
    ...
    % 主问题增加约束
    MP_cons = [MP_cons, alpha >= c'*y_k(:,k+1);
        D*y_k(:,k+1) >= d;
        K*y_k(:,k+1) == g;
        F*x + G*y_k(:,k+1) >= h
        I_u*y_k(:,k+1) == u_k ;
