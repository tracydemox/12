...
for k=1:itr
 ...
 %+++++++++++++++++++++++++ 强对偶求解    
 %     第二阶段问题目标函数
 %         SP_Obj = d'*gamma + g'*lamda + (h-F*x_k)'*nu + u'*pai;
 SP_Obj = d'*gamma + g'*lamda + (h-F*x_k)'*nu + u_hat'*pai + du'*BB;
 %     第二阶段问题约束
 SP_cons = [SP_cons,  D'*gamma + K'*lamda + G'*nu + I_u'*pai <= c, ...
     gamma >=0, nu >= 0];
%  SP_cons = [SP_cons, -M <= lamda <= M, -M <= pai <= M];  % Add bounds for equality duals
 SP_cons = [SP_cons, sum(B_pv) <= Gamma_pv, sum(B_L) <= Gamma_L];
 SP_cons = [SP_cons, -M*B<=BB<=M*B, pai-M*(1-B)<=BB<=pai+M*(1-B)];
